            ...
            function (
                ...
                $scope.saveData = function (detalleCredito) {
                    var datos = {
                        ...
                        deudatotal:
                            detalleCredito.monto_id.monto +
                            detalleCredito.interes,
                        cuota_diaria: detalleCredito.cuota_diaria,
                        cuota_minima:
                            detalleCredito.monto_id.monto /
                            detalleCredito.planes_id.dias,
                        ...
                        $http({
                            ...
                            function successCallback(response) {
                                if (response.data.result) {
                                    ...
                                    $scope.detalle_cliente.nombre =
                                        response.data.records.nombre +
                                        " " +
                                        response.data.records.apellido;
                ...
                $scope.addNewCredit = function (cliente) {
                    ...
                    $scope.detalle_cliente.credito =
                        cliente.statusCredit == 2 ? 1 : 0;
                    $scope.detalle_cliente.nombre =
                ...
                $scope.optionYes = function () {
                    pdfsService.debtrecognition(167);
                    modal.close();
                };
                ...
                function getEndDateWeek(plan) {
                    var startDate = getStartDate();
                    startDate.setDate(
                        startDate.getDate() + (plan.dias - 1) * 7
                    );
                    return $filter("date")(startDate, "dd-MM-yyyy");
                }
                ...
                $scope.modalReconocimientoDeuda = function (customer) {
                    ...
                    modal = $modal.open({
                        templateUrl:
                            "views/creditos/modalReconocimientoDeuda.html",
                        scope: $scope,
                        size: "md",
                        resolve: function () {},
                        windowClass: "default",
